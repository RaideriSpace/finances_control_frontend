...
import { formatarMoeda, formatarData } from "@/app/lib/format";
...
export function ListaTransacoes({ initialData }: ListaTransacoesProps) {
	...
	// Mantém a lista em sincronia quando o Server Component reenvia dados
	// frescos após um `router.refresh()` (ex.: depois de editar um lançamento).
	// Ajuste de estado durante a renderização (sem efeito) — ver
	// https://react.dev/learn/you-might-not-need-an-effect#adjusting-some-state-when-a-prop-changes
	const [dadosSincronizados, setDadosSincronizados] = useState(initialData);
	if (initialData !== dadosSincronizados) {
		setDadosSincronizados(initialData);
		setTransacoes(initialData);
	}
	...
	const handleDelete = async (transacao: Transacao) => {
		const confirmado = await confirm("Deseja deletar permanentemente esta transação?", { title: "Excluir lançamento" });
	...
	const transacoesPorDia = useMemo(() => {
		...
		for (const transacao of transacoesFiltradas) {
			const dia = transacao.data_pagamento?.slice(0, 10) ?? "sem-data";
			grupos.set(dia, [...(grupos.get(dia) ?? []), transacao]);
		}
	...
	// Classes completas (não interpoladas) por cartão — o Tailwind JIT só
	// consegue detectar classes que aparecem como texto literal no arquivo;
	// a versão anterior montava `text-${cor} bg-${cor}` em runtime, o que o
	// purge não conseguia enxergar e podia sumir em produção.
	const cartaoTagClass: Record<TransacaoCartao, string> = {
		picpay: "text-black bg-positive",
		inter: "text-black bg-secondary",
		mercado_pago: "text-black bg-auxiliary1",
		amazon: "text-black bg-auxiliary2",
		swile: "text-black bg-tertiary",
		nubank: "text-black bg-primary-light",
		outro: "text-white bg-tertiary-ex-dark",
	};
		...
		<>
			<section className="mt-l space-y-m">
				...
				:	<div className="space-y-m">
						{transacoesPorDia.map(({ dia, itens }) => (
							<section key={dia}>
								<div className="mb-xs flex items-center gap-s">
									<span className="text-xs font-bold uppercase tracking-widest text-primary-ex-light">
										{dia === "sem-data" ? "Sem data" : formatarData(dia)}
									</span>
									<span className="h-px flex-1 bg-dark-light" />
									<span className="text-[10px] text-auxiliary2-light">
										{itens.length} lançamento{itens.length === 1 ? "" : "s"}
									</span>
								</div>
								<div className="flex flex-col gap-[2px]">
									{itens.map((t, index) => {
										const isNegative = !ehEntrada(t.acao);

										const borderColor = cartaoBorderColor[t.cartao] ?? "border-x-dark-light";
										const tagClass = cartaoTagClass[t.cartao] ?? "text-white bg-dark-light";
									...
									`}>
												...
												<div className="flex-1 min-w-0">
													...
													<div className="flex items-center gap-xs mt-[3px] flex-wrap">
														...
														<span
															className={`text-[10px] font-bold uppercase tracking-wide px-xs py-[2px] rounded border border-primary/30 ${tagClass}`}>
															{t.cartao}
														</span>
